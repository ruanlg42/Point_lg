...
class TimeTransformer(nn.Module):
    ...
    def __init__(self, d_model=128, nhead=8, num_layers=4, dim_feedforward=512, dropout=0.2,
        ...
        # Input projection: 4-channel physics features -> d_model dimension
    ...
    def forward(self, temp_sequence):
        ...
        # Input projection [batch, seq_len, 4] -> [batch, seq_len, d_model]
...
class CNN1D(nn.Module):
    ...
    def forward(self, temp_sequence):
        ...
        Returns:
            e_pred: [batch, 1] predicted e value
        """
        # 预处理: [batch, seq_len] -> [batch, 4, seq_len]
        x = self._preprocess_sequence(temp_sequence)
...
class PhysicsInformedCNN(nn.Module):
    ...
    def preprocess_sequence(self, temp_sequence):
        """
        Preprocess temperature sequence into 4-channel input (with Gaussian filtering)
        ...
        t_inv_sqrt = 1.0 / torch.sqrt(time_steps.clamp_min(1e-12))  # [seq_len]
        t_inv_sqrt = t_inv_sqrt.unsqueeze(0).expand(batch_size, -1)  # [batch, seq_len]
...
class TimeVAE1D_Mamba(nn.Module):
    ...
    def __init__(self, seq_len=200, C_in=1, latent_dim=64,
                 ...
                 decoder_base=64, total_time=5.0, delta_t=0.02):
        """
        Args:
            seq_len: 输入序列长度
    ...
    def encode(self, x):
        ...
        """
        # 注意：这里假设输入x已经是滤波后的数据（在forward中已预处理）
    ...
    def forward(self, x):
        ...
        Args:
            x: [batch, C_in, seq_len]
        Returns:
            recon: [batch, 1, seq_len] 重建序列（重建的是滤波后的数据）
            e_pred: [batch, 1] 预测的e参数
            (mu, logvar): 隐变量分布参数（用于计算KL散度）
            x_smooth: [batch, 1, seq_len] 滤波后的原始数据（用于损失计算，可选）
        """
        ...
        # 预处理：获取滤波后的数据
        if x.size(1) == 1:
            temp_seq = x.squeeze(1)  # [batch, seq_len]
            temp_smooth = self._preprocess_sequence(temp_seq)  # [batch, seq_len] 滤波后的数据
            x_smooth = temp_smooth.unsqueeze(1)  # [batch, 1, seq_len]
        else:
            x_smooth = x  # 如果已经是多通道，直接使用
        
        mu, logvar = self.encode(x_smooth)  # 使用滤波后的数据进行编码
        z = self.reparameterize(mu, logvar)
        recon = self.decode(z)
        e_pred = self.param_head(z)
        
        # 返回重建序列、预测、隐变量参数，以及滤波后的原始数据（用于损失计算）
        return recon, e_pred, (mu, logvar), x_smooth
...
class TimeVAE1D_Transformer(nn.Module):
    ...
    def __init__(self, seq_len=200, C_in=1, latent_dim=64,
                 ...
                 dropout=0.2, decoder_base=64, total_time=5.0, delta_t=0.02):
        """
        Args:
            seq_len: 输入序列长度
            C_in: 输入通道数（默认1，原始温度序列）
        ...
        # ------ Encoder: Transformer (只做高斯滤波，保持1通道) ------
...
class TimeVAE1D_HybridMambaTransformer(nn.Module):
    ...
    def __init__(self, seq_len=200, C_in=1, latent_dim=64,
        ...
        self.total_time = total_time
        self.delta_t = delta_t
        self.decoder_base = decoder_base  # 保存decoder_base参数

        # ------ Encoder: Hybrid Mamba-Transformer (只做高斯滤波，保持1通道) ------
    ...
    def encode(self, x):
        ...
        """
        x_preprocessed = x
        _, h_global = self.encoder(x_preprocessed)  # [batch, d_model]
    ...
    def decode(self, z):
        ...
        # 线性投影到固定长度64
        h = self.fc_dec(z).view(B, self.decoder_base, 64)
...
class TimeVAE1D_HybridMT_Physics4Channel(nn.Module):
    """
    基于Hybrid Mamba-Transformer的时间序列VAE，使用4通道物理特征输入
    - 输入: 4通道物理特征 [T, Ṫ, t^(-1/2), Δt]，形状为 [batch, 4, seq_len]
    - 重建: 只重建滤波后的温度序列 T（第0通道）
    - 重建loss: 用滤波后的曲线进行对比
    
    优势：
    - 多通道物理先验特征，增强模型对物理规律的感知
    - 重建loss专注于滤波后的温度曲线，更稳定
    - 使用Hybrid Mamba-Transformer编码器，结合Mamba的效率和Transformer的全局建模能力
    """
    def __init__(self, seq_len=200, C_in=4, latent_dim=64,
                 d_model=128, n_mamba=2, n_transformer=2, nhead=8,
                 d_state=16, d_conv=4, expand=2, dropout=0.2, decoder_base=64,
                 total_time=5.0, delta_t=0.02):
        """
        Args:
            seq_len: 输入序列长度
            C_in: 输入通道数（固定为4：T, Ṫ, t^(-1/2), Δt）
            latent_dim: 隐空间维度
            d_model: 模型维度
            n_mamba: Mamba层数 (底层，快速局部建模)
            n_transformer: Transformer层数 (顶层，全局精细化)
            nhead: Transformer注意力头数
            d_state: SSM状态维度
            d_conv: 局部卷积宽度
            expand: 扩展因子
        ...
        # ------ Encoder: Hybrid Mamba-Transformer (输入4通道物理特征) ------
        self.encoder = HybridMambaTransformerEncoder(
            C_in=C_in,
            d_model=d_model,
            n_mamba=n_mamba,
        ...
        # ------ Decoder: 使用自适应上采样回到seq_len，只重建温度T ------
        self.fc_dec = nn.Linear(latent_dim, decoder_base * 64)  # 固定起始长度64
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Conv1d(decoder_base, 32, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.Conv1d(32, 1, kernel_size=3, padding=1)  # -> [batch, 1, seq_len] 只重建温度T
        )
    ...
    def _preprocess_sequence_to_4channels(self, temp_sequence):
        """
        从原始温度序列提取4通道物理特征
        ...
        Args:
            temp_sequence: [batch, seq_len] 原始温度序列
        Returns:
            features: [batch, 4, seq_len] 4通道物理特征
                Channel 0: T (滤波后的温度)
                Channel 1: Ṫ (温度导数)
                Channel 2: t^(-1/2) (物理先验)
                Channel 3: Δt (归一化的采样间隔)
            T_smooth: [batch, seq_len] 滤波后的温度（用于重建loss）
        """
        batch_size, seq_len = temp_sequence.shape
        device = temp_sequence.device
        ...
        # Channel 0: T (滤波后的温度)
        T = T_smooth  # [batch, seq_len]
        
        # Channel 1: Ṫ (温度导数，数值微分)
        T_dot = torch.zeros_like(T_smooth)
        T_dot[:, :-1] = (T_smooth[:, 1:] - T_smooth[:, :-1]) / self.delta_t
        T_dot[:, -1] = T_dot[:, -2]  # 最后一个点复制前一个
        
        # Channel 2: t^(-1/2) (半无限体扩散先验)
        time_steps = torch.arange(0, seq_len, device=device, dtype=torch.float32) * self.delta_t
        time_steps[0] = self.delta_t  # 避免除零
        t_inv_sqrt = 1.0 / torch.sqrt(time_steps.clamp_min(1e-12))
        t_inv_sqrt = t_inv_sqrt.unsqueeze(0).expand(batch_size, -1)  # [batch, seq_len]
        ...
        # Stack all channels: [batch, 4, seq_len]
        features = torch.stack([T, T_dot, t_inv_sqrt, delta_t_normalized], dim=1)
        
        return features, T_smooth

    def encode(self, x):
        """
        编码器：输入4通道物理特征 -> 隐空间均值和方差
        
        Args:
            x: [batch, 4, seq_len] 4通道物理特征
        Returns:
            mu: [batch, latent_dim] 隐变量均值
            logvar: [batch, latent_dim] 隐变量对数方差
        """
    ...
    def decode(self, z):
        ...
        Args:
            z: [batch, latent_dim]
        Returns:
            xhat: [batch, 1, seq_len] 重建的温度序列（滤波后的）
        """
        B = z.size(0)
        decoder_base = 64  # 与 __init__ 中的 decoder_base 参数一致
    ...
    def forward(self, x):
        ...
        Args:
            x: [batch, C_in, seq_len]
                - 如果 C_in=1: 原始温度序列，会自动转换为4通道物理特征
                - 如果 C_in=4: 已经是4通道物理特征，直接使用
        Returns:
            recon: [batch, 1, seq_len] 重建序列（重建的是滤波后的温度T）
            e_pred: [batch, 1] 预测的e参数
            (mu, logvar): 隐变量分布参数（用于计算KL散度）
            x_smooth: [batch, 1, seq_len] 滤波后的温度数据（用于重建loss计算）
        """
        assert x.dim() == 3 and x.size(2) == self.seq_len, \
            f"输入应为 [batch, C_in, seq_len]，且seq_len={self.seq_len}，实际得到 {x.shape}"
        
        # 预处理：如果输入是1通道（原始温度），转换为4通道物理特征
        if x.size(1) == 1:
            temp_seq = x.squeeze(1)  # [batch, seq_len]
            x_4ch, T_smooth = self._preprocess_sequence_to_4channels(temp_seq)  # [batch, 4, seq_len], [batch, seq_len]
            x_smooth = T_smooth.unsqueeze(1)  # [batch, 1, seq_len] 用于重建loss
        elif x.size(1) == 4:
            # 已经是4通道，直接使用
            x_4ch = x  # [batch, 4, seq_len]
            # 从第0通道获取滤波后的温度（用于重建loss）
            x_smooth = x_4ch[:, 0:1, :]  # [batch, 1, seq_len]
        else:
            raise ValueError(f"输入通道数应为1或4，实际得到 {x.size(1)}")
        
        mu, logvar = self.encode(x_4ch)  # 使用4通道物理特征进行编码
        z = self.reparameterize(mu, logvar)
        recon = self.decode(z)  # 重建滤波后的温度T
        e_pred = self.param_head(z)
        
        # 返回重建序列、预测、隐变量参数，以及滤波后的温度数据（用于损失计算）
        return recon, e_pred, (mu, logvar), x_smooth


class TwoStageSharedMambaEncoder(nn.Module):
    """
    两阶段非均匀分段编码器（Two-Stage Shared-Mamba Encoder）V2
    
    核心设计：
    - 阶段1 (0-1s): 快速升温阶段，信息量最大
    - 阶段2 (1-5s): 热扩散+平衡阶段，时间更长
    
    参数共享策略：
    - Mamba层：两个阶段共享（捕获通用的局部时序模式）
    - Transformer层：每个阶段独立（专门化建模不同物理过程）
    
    架构改进（V2）：
    ✅ 位置编码：可学习的位置嵌入，保留时序位置信息
    ✅ Mamba残差连接：Pre-LN + Residual + Dropout，训练更稳定
    ✅ 规范化流程：Mamba (Pre-LN) → Transformer (Post-LN) → Final LN
    
    优势：
    - 减少参数量（Mamba层只有一份）
    - 保持专门化能力（Transformer层独立）
    - 非均匀划分更符合物理过程
    - 位置信息完整保留（解决Transformer丢失位置问题）
    - 训练稳定性提升（残差+规范化）
    """
    def __init__(self, C_in=1, d_model=128, n_mamba=2, n_transformer=2, 
                 nhead=8, d_state=16, d_conv=4, expand=2, dropout=0.2,
        ...
        self.total_time = total_time
        self.delta_t = delta_t
        self.stage1_time = 1.0  # 第一阶段：0-1s
        
        # 输入投影（共享）
        self.input_proj = nn.Conv1d(C_in, d_model, kernel_size=1)
        
        # ============ 位置编码（可学习，共享于两个阶段）============
        # 使用可学习的位置编码，长度设为足够长以覆盖最长的阶段
        max_seq_len = int(total_time / delta_t) + 10  # 留出余量
        self.pos_embed = nn.Parameter(torch.randn(1, max_seq_len, d_model) * 0.02)
        
        # ============ 共享的Mamba层（带残差和LayerNorm）============
        self.shared_mamba_layers = nn.ModuleList()
        self.mamba_norms = nn.ModuleList()
        self.mamba_dropouts = nn.ModuleList()
        
        for _ in range(n_mamba):
            self.shared_mamba_layers.append(
                Mamba(
                    d_model=d_model,
                    d_state=d_state,
                    d_conv=d_conv,
            ...
            self.mamba_dropouts.append(nn.Dropout(dropout))
        
        # ============ 阶段1独立Transformer层 (0-1s) ============
        self.stage1_transformer_layers = nn.ModuleList([
        ...
        # ============ 阶段2独立Transformer层 (1-5s) ============
        self.stage2_transformer_layers = nn.ModuleList([
            nn.TransformerEncoderLayer(
                d_model=d_model,
                nhead=nhead,
                dim_feedforward=d_model * 4,
        ...
        self.stage_attention = nn.Sequential(
            ...
            nn.Dropout(dropout),
            nn.Linear(128, 2),
            nn.Softmax(dim=-1)  # [batch, 2] 两个阶段的权重
        )
    ...
    def _encode_stage(self, x, transformer_layers, norm):
        """
        对单个阶段进行编码
        
        Args:
            x: [batch, C_in, seq_len] 输入序列片段
            transformer_layers: 该阶段的Transformer层
            norm: 该阶段的LayerNorm
        Returns:
            h_global: [batch, d_model] 全局表示
        """
        # 输入投影
        h = self.input_proj(x)  # [batch, d_model, seq_len]
        
        # 转换为 [batch, seq_len, d_model] 供Mamba使用
        h = h.transpose(1, 2)  # [batch, seq_len, d_model]
        seq_len = h.size(1)
        
        # ============ 添加位置编码 ============
        # 使用前seq_len个位置编码，broadcast到batch维度
        h = h + self.pos_embed[:, :seq_len, :]  # [batch, seq_len, d_model]
        
        # ============ 共享Mamba层（带残差连接和LayerNorm）============
        # Mamba期望输入: [batch, seq_len, d_model]
        for mamba_layer, mamba_norm, mamba_dropout in zip(
            self.shared_mamba_layers, self.mamba_norms, self.mamba_dropouts
        ):
            # Pre-LN + Residual连接
            h_residual = h
            h_normed = mamba_norm(h)
            h_mamba = mamba_layer(h_normed)  # [batch, seq_len, d_model]
            h_mamba = mamba_dropout(h_mamba)
            h = h_residual + h_mamba  # 残差连接
        
        # ============ 独立Transformer层（全局+阶段专门化）============
        # Transformer内部已有残差和LayerNorm
        for transformer_layer in transformer_layers:
            h = transformer_layer(h)  # [batch, seq_len, d_model]
        
        # 最终LayerNorm
        h = norm(h)  # [batch, seq_len, d_model]
    ...
    def forward(self, x):
        """
        Args:
            x: [batch, C_in, seq_len] 完整序列
        Returns:
            h_global: [batch, d_model] 融合后的全局表示
            stage_weights: [batch, 2] 每个阶段的权重（用于可视化）
        """
        batch_size, C_in, seq_len = x.shape
        
        # 非均匀分段（基于时间）
        # 阶段1: 0-1s，阶段2: 1-5s
        stage1_len = int(self.stage1_time / self.delta_t)  # 约30个点（1s / 0.033s）
        stage1_len = min(stage1_len, seq_len)  # 防止超出范围
        
        x_stage1 = x[:, :, :stage1_len]           # [batch, C_in, ~30] (0-1s)
        x_stage2 = x[:, :, stage1_len:]           # [batch, C_in, ~121] (1-5s)
        
        # 分别编码两个阶段（共享Mamba，独立Transformer）
        h_stage1 = self._encode_stage(x_stage1, self.stage1_transformer_layers, self.stage1_norm)
        h_stage2 = self._encode_stage(x_stage2, self.stage2_transformer_layers, self.stage2_norm)
        
        # 拼接所有阶段的特征
        h_concat = torch.cat([h_stage1, h_stage2], dim=-1)  # [batch, d_model * 2]
        
        # 学习每个阶段的重要性权重
        stage_weights = self.stage_attention(h_concat)  # [batch, 2]
        
        w = stage_weights
        h_weighted = h_stage1 * w[:, 0:1] + h_stage2 * w[:, 1:2]    # [B, d_model]
        h_fused = self.fusion(torch.cat([h_weighted, h_concat], dim=-1))  # 输入维=3*d_model

        
        return h_fused, stage_weights


class TimeVAE1D_StageAware(nn.Module):
    """
    时序分段感知VAE (Stage-Aware VAE) V2 - 两阶段版本
    ...
    - 将温度序列分为两个物理阶段（非均匀分段）
      ...
      * 阶段2 (1-5s): 热扩散+平衡阶段，时间更长
    - Mamba层共享（捕获通用时序模式）
    - Transformer层独立（专门化建模不同物理过程）
    - 自适应学习每个阶段的重要性权重
    
    V2架构改进：
    ✅ 位置编码：可学习的位置嵌入，保留时序位置信息
    ✅ Mamba残差连接：Pre-LN + Residual + Dropout，训练更稳定
    ✅ 规范化流程：完整的Pre-LN和Post-LN组合
    
    优势：
    - 参数更少（Mamba层共享）
    - 物理意义更强（非均匀分段符合实际过程）
    - 专门化能力强（Transformer层独立）
    - 只使用1通道输入，简单高效
    - 训练稳定性提升（V2改进）
    
    输入: [batch, 1, seq_len] 温度序列（只做高斯滤波）
    输出: 重建 [batch, 1, seq_len]、参数e预测 [batch, 1]、阶段权重 [batch, 2]
    """
    def __init__(self, seq_len=251, C_in=1, latent_dim=64,
                 d_model=128, n_mamba=2, n_transformer=2, nhead=8,
                 d_state=16, d_conv=4, expand=2, dropout=0.2, decoder_base=64,
                 total_time=5.0, delta_t=0.02):
        """
        Args:
            ...
            d_model: 模型维度
            n_mamba: Mamba层数（共享）
            n_transformer: Transformer层数（每个阶段独立）
            nhead: Transformer注意力头数
            d_state: SSM状态维度
            d_conv: Mamba局部卷积宽度
            expand: Mamba扩展因子
        ...
        # ------ Encoder: Two-Stage Shared-Mamba编码器 ------
        self.encoder = TwoStageSharedMambaEncoder(
            C_in=C_in,
            d_model=d_model,
            n_mamba=n_mamba,
            n_transformer=n_transformer,
    ...
    def encode(self, x):
        ...
        Returns:
            ...
            stage_weights: [batch, 2] 每个阶段的权重（阶段1和阶段2）
        """
        h_global, stage_weights = self.encoder(x)  # [batch, d_model], [batch, 2]
        mu = self.fc_mu(h_global)
        logvar = self.fc_logvar(h_global)
        return mu, logvar, stage_weights
    ...
    def forward(self, x):
        ...
        Args:
            x: [batch, 1, seq_len] 温度序列
        Returns:
            recon: [batch, 1, seq_len] 重建序列
            e_pred: [batch, 1] 预测的e参数
            (mu, logvar): 隐变量分布参数
            x_smooth: [batch, 1, seq_len] 滤波后的数据
            stage_weights: [batch, 2] 阶段权重（阶段1: 0-1s, 阶段2: 1-5s，用于分析）
        """
        assert x.dim() == 3 and x.size(2) == self.seq_len, \
            f"输入应为 [batch, 1, seq_len]，且seq_len={self.seq_len}，实际得到 {x.shape}"
        
        # 预处理：高斯滤波
        if x.size(1) == 1:
            temp_seq = x.squeeze(1)  # [batch, seq_len]
            temp_smooth = self._preprocess_sequence(temp_seq)
            x_smooth = temp_smooth.unsqueeze(1)  # [batch, 1, seq_len]
        else:
            x_smooth = x
        
        # 编码（两阶段分段）
        mu, logvar, stage_weights = self.encode(x_smooth)
        ...
        # 返回所有必要信息（stage_weights现在是2个值）
        return recon, e_pred, (mu, logvar), x_smooth, stage_weights


class PositionalEncoding_SST(nn.Module):
    """正余弦位置编码：输入/输出 [B, T, D] 形状不变"""
...
class PatchEmbed1D(nn.Module):
    """
    1D patch embedding：Conv1d 替代 unfold
    输入 [B, C, L] → Conv1d(C→D, k=patch_len, s=stride) → [B, D, T] → 转置 → [B, T, D]
    """
    def __init__(self, c_in: int, d_model: int, patch_len: int, stride: int):
        super().__init__()
        assert patch_len >= 1 and stride >= 1
...
class MambaStack(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        h = x
        for layer, ln, do in zip(self.layers, self.norms, self.dropouts):
            res = h
            h = layer(h)   # [B, T, D]
            h = do(h)
            h = ln(h + res)
...
class TransformerStack(nn.Module):
    """Transformer Encoder 堆叠（batch_first=True），输入输出 [B, T, D]"""
    def __init__(self, d_model: int, n_layers: int = 2, n_heads: int = 8,
                 d_ff: int = None, dropout: float = 0.1, use_posenc: bool = True):
        super().__init__()
        d_ff = d_ff or (4 * d_model)
        self.use_posenc = use_posenc
        if use_posenc:
            self.pe = PositionalEncoding_SST(d_model)
...
class Router(nn.Module):
    """
    基于输入 [B, L, C] 的路由器，输出 (m_weight, t_weight) ∈ [0,1] 且和为 1：
      - m_weight：长程（Mamba）分支权重
      - t_weight：短程（Transformer）分支权重
    """
    def __init__(self, c_in: int, d_model: int, long_len: int):
        super().__init__()
        self.proj = nn.Linear(c_in, d_model)
        self.flat = nn.Flatten(start_dim=1)             # 展平 L*D
        self.head = nn.Linear(long_len * d_model, 2)
        self.sm   = nn.Softmax(dim=-1)

    def forward(self, x_long: torch.Tensor):
        # x_long: [B, L, C]
        h = self.proj(x_long)         # [B, L, D]
        h = self.flat(h)              # [B, L*D]
        prob = self.sm(self.head(h))  # [B, 2]
        m_weight = prob[:, 0]         # [B]
        t_weight = prob[:, 1]         # [B]
        return m_weight, t_weight


class NewSSTEncoder_Weighted(nn.Module):
    """
    仅 Weighted 融合的改进版 SST Encoder（不含 RevIN）
    输入:  x ∈ [B, L, C] 或 [B, L]（单通道）
    输出:  h_tokens ∈ [B, T_token, D']（可选），h_global ∈ [B, D']
    设计:
      - Long 分支: PatchEmbed1D → MambaStack
      - Short 分支: 取最近 context_window，PatchEmbed1D → TransformerStack
      - Router: 产生 (m_weight, t_weight)
      - 融合: Weighted（加权相加）
      - 可选 tokens 插值回 L（upsample_to=L）
    """
    def __init__(
        self,
        c_in: int = 1,
        d_model: int = 128,
        long_patch_len: int = 16,
        long_stride: int = 8,
        short_patch_len: int = 16,
        short_stride: int = 8,
        context_window: int = 151,     # 短程分支使用的近期长度
        n_mamba: int = 2,
        d_state: int = 16,
        d_conv: int = 4,
        m_expand: int = 2,
        ...
        project_to: int = None,  # 输出维 D'
        return_tokens: bool = True,
        upsample_to: int = None  # 若不为 None，把 token 插值到该长度（如 151）
    ):
        super().__init__()
        self.c_in = c_in
        self.d_model = d_model
        self.project_to = project_to or d_model
        self.context_window = context_window
        self.use_router = use_router
        self.return_tokens = return_tokens
        self.upsample_to = upsample_to

        # Router（延迟构建：forward 里在知道 L 后再初始化）
        self.router = None

        # Long 分支
        self.long_patch = PatchEmbed1D(c_in=c_in, d_model=d_model,
                                       patch_len=long_patch_len, stride=long_stride)
        self.long_mamba = MambaStack(d_model=d_model, n_layers=n_mamba,
                                     d_state=d_state, d_conv=d_conv,
                                     expand=m_expand, dropout=dropout)

        # Short 分支
        self.short_patch = PatchEmbed1D(c_in=c_in, d_model=d_model,
                                        patch_len=short_patch_len, stride=short_stride)
        self.short_tr = TransformerStack(d_model=d_model, n_layers=n_transformer,
                                         n_heads=n_heads, d_ff=d_ff,
                                         dropout=dropout, use_posenc=True)

        # 将每支 token 映射到 D'
        self.to_dprime = nn.Linear(d_model, self.project_to)

    def _maybe_build_router(self, L: int):
        if (self.router is None) and self.use_router:
            self.router = Router(c_in=self.c_in, d_model=self.d_model, long_len=L)
            # 将 router 移动到与输入相同的设备
            if hasattr(self, '_device'):
                self.router = self.router.to(self._device)

    @staticmethod
    def _tokens_interp(tokens: torch.Tensor, tgt_T: int) -> torch.Tensor:
        # tokens: [B, T0, D] → 插值到 tgt_T（沿时间维）
        if tokens.size(1) == tgt_T:
            return tokens
        return F.interpolate(tokens.transpose(1, 2), size=tgt_T,
    ...
    def forward(self, x: torch.Tensor):
        ...
        Args:
            x: [B, L, C] 或 [B, L]
        Returns:
            h_tokens: [B, T_token, D']（或 None）
            h_global: [B, D']
        """
        if x.dim() == 2:
            x = x.unsqueeze(-1)  # [B,L] → [B,L,1]
        B, L, C = x.shape
        assert C == self.c_in, f"C_in不匹配：期望{self.c_in}，实际{C}"
        ...
        # Router（根据原尺度统计得到权重）
        if self.use_router:
            self._maybe_build_router(L)
            if self.router is not None:
                self.router = self.router.to(x.device)
            m_weight, t_weight = self.router(x)  # [B], [B]
        else:
            # 若不用 Router，默认为 0.5/0.5
            m_weight = torch.full((B,), 0.5, device=x.device, dtype=x.dtype)
            t_weight = torch.full((B,), 0.5, device=x.device, dtype=x.dtype)

        # ===== Long 分支 =====
        x_long = x.transpose(1, 2)               # [B, C, L]
        tl = self.long_patch(x_long)             # [B, Tm, D]
        tl = self.long_mamba(tl)                 # [B, Tm, D]
        tl = self.to_dprime(tl)                  # [B, Tm, D']

        # ===== Short 分支（取近期 context_window）=====
        cw = min(self.context_window, L)
        x_short = x[:, -cw:, :].transpose(1, 2)  # [B, C, cw]
        ts = self.short_patch(x_short)           # [B, Ts, D]
        ts = self.short_tr(ts)                   # [B, Ts, D]
        ts = self.to_dprime(ts)                  # [B, Ts, D']

        # ===== Weighted 融合：时间维对齐到相同长度后加权相加 =====
        T = max(tl.size(1), ts.size(1))
        tl_aln = self._tokens_interp(tl, T)      # [B, T, D']
        ts_aln = self._tokens_interp(ts, T)      # [B, T, D']

        mw = m_weight.view(B, 1, 1)              # [B,1,1]
        tw = t_weight.view(B, 1, 1)              # [B,1,1]
        h_tokens = mw * tl_aln + tw * ts_aln     # [B, T, D']

        # 可选：插值回原长度（如 L=151），便于与逐时刻模块对齐
        if (self.upsample_to is not None) and self.return_tokens:
            h_tokens = self._tokens_interp(h_tokens, self.upsample_to)

        # 全局池化
        h_global = h_tokens.mean(dim=1)          # [B, D']

        if self.return_tokens:
            return h_tokens, h_global
        else:
            return None, h_global


class TimeVAE1D_SSTEncoder_Residual(nn.Module):
    """
    基于 NewSSTEncoder_Weighted 的时间序列 VAE - 残差重建版本
    
    核心改进：
    - 使用新的 SST Encoder（Long分支Mamba + Short分支Transformer + 自适应路由）
    - 残差重建 + 初值约束
    - 解码器预测残差 Δx = decoder(z)
    - 最终输出 x̂ = x0_smooth + Δx（以初温为基线）
    - 初值透传：强制 x̂[0] = x0_smooth
    - 初值约束损失 L_ic = ||Δx(t=0)||₁ ≈ 0
    
    优势：
    - SST Encoder 结合长短期建模优势
    - 自适应路由器动态调整分支权重
    - 初始温度天然"透传"，解码器只需补细节
    - 降低解码器学习难度，更快收敛
    - 适合温度序列等具有明确初值的物理过程
    ...
    架构：
    - Encoder: NewSSTEncoder_Weighted (Long Mamba + Short Transformer + Router)
    - Decoder: Conv1D (上采样卷积，预测相对于初温的残差)
    
    输入: [batch, C_in, seq_len]
    输出: 重建 [batch, 1, seq_len]、参数e预测 [batch, 1]
    """
    def __init__(self, seq_len=251, C_in=1, latent_dim=64,
                 d_model=128, long_patch_len=16, long_stride=8,
                 short_patch_len=16, short_stride=8, context_window=151,
                 n_mamba=2, n_transformer=2, nhead=8,
                 d_state=16, d_conv=4, expand=2, dropout=0.2, 
                 decoder_base=64, use_router=True,
                 total_time=5.0, delta_t=0.02):
        """
        Args:
            seq_len: 输入序列长度
            C_in: 输入通道数（默认1，原始温度序列）
            latent_dim: 隐空间维度
            d_model: SST Encoder模型维度
            long_patch_len: Long分支patch长度
            long_stride: Long分支stride
            short_patch_len: Short分支patch长度
            short_stride: Short分支stride
            context_window: Short分支使用的近期窗口长度
            n_mamba: Mamba层数（Long分支）
            n_transformer: Transformer层数（Short分支）
            nhead: Transformer注意力头数
            d_state: SSM状态维度
            d_conv: Mamba局部卷积宽度
            expand: Mamba扩展因子
        ...
        # ------ Encoder: NewSSTEncoder_Weighted (高斯滤波后，保持1通道) ------
        self.encoder = NewSSTEncoder_Weighted(
            c_in=C_in,
            d_model=d_model,
            long_patch_len=long_patch_len,
            long_stride=long_stride,
            short_patch_len=short_patch_len,
        ...
        # ------ Decoder: 预测相对于初温的残差 ------
        # 输入：latent_dim + 1（初始温度条件）
        self.fc_dec = nn.Linear(latent_dim + 1, decoder_base * 64)
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Conv1d(decoder_base, 32, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.Conv1d(32, 1, kernel_size=3, padding=1)  # -> [batch, 1, seq_len] 残差
        )
    ...
    def encode(self, x):
        ...
        Returns:
            ...
            logvar: [batch, latent_dim] 隐变量对数方差
        """
        # 转换为 [batch, seq_len, C_in] 格式（SST Encoder期望的格式）
        x_sst = x.transpose(1, 2)  # [batch, seq_len, C_in]
        
        # SST Encoder: 返回 (h_tokens, h_global)，这里只用 h_global
        _, h_global = self.encoder(x_sst)  # [batch, d_model]
    ...
    def decode(self, z, x_skip=None):
        """
        解码器：(隐变量 + 初始温度) -> 以初温为基线的残差 -> 重建 -> 初值透传
        
        核心思想：
        1. 初始温度作为条件输入给decoder
        2. Decoder预测相对于初始温度的残差曲线（从0开始）
        3. 重建：x̂ = x0 + Δx（初温 + 残差）
        4. 透传：强制 x̂[0] = x0
        
        Args:
            z: [batch, latent_dim]
            x_skip: [batch, 1, seq_len] 滤波后的输入序列（用于提取初始温度）
        Returns:
            xhat: [batch, 1, seq_len] 重建的序列（初值已透传）
            x_delta: [batch, 1, seq_len] 预测的残差（相对于初温，用于初值约束）
        """
        B = z.size(0)
        
        if x_skip is None:
            # 如果没有x_skip，无法提取初始温度，使用零作为默认值
            h = self.fc_dec(torch.cat([z, torch.zeros(B, 1, device=z.device)], dim=1))
            h = h.view(B, self.decoder_base, 64)
            x_delta = self.decoder(h)
            return x_delta, x_delta
        
        # 提取滤波后序列的初始温度 [batch, 1]
        x0_smooth = x_skip[:, :, 0]  # [batch, 1]
        
        # 拼接隐变量和初始温度作为条件：[batch, latent_dim + 1]
        z_with_ic = torch.cat([z, x0_smooth], dim=1)
        
        # 线性投影到固定长度64
        h = self.fc_dec(z_with_ic).view(B, self.decoder_base, 64)
        
        # 通过解码器预测残差（相对于初始温度的变化）
        x_delta = self.decoder(h)  # [batch, 1, seq_len]
        
        # 以初始温度为基线重建：x̂ = x0 + Δx
        # 将初始温度广播到整个序列
        x0_broadcast = x0_smooth.unsqueeze(2).expand(B, 1, self.seq_len)  # [batch, 1, seq_len]
        xhat_base = x0_broadcast + x_delta  # [batch, 1, seq_len]
        
        # 初值透传：强制 x̂[0] = x0（即使 Δx[0] != 0）
        xhat = xhat_base.clone()
        xhat[:, :, 0] = x0_smooth
    ...
    def forward(self, x):
        ...
        Returns:
            ...
            e_pred: [batch, 1] 预测的e参数
            (mu, logvar): 隐变量分布参数（用于计算KL散度）
            x_smooth: [batch, 1, seq_len] 滤波后的输入（用于损失计算）
        ...
        mu, logvar = self.encode(x_smooth)  # 使用滤波后的数据进行编码
        z = self.reparameterize(mu, logvar)
        recon, x_delta = self.decode(z, x_skip=x_smooth)  # 残差重建
        e_pred = self.param_head(z)
        
        # 返回重建序列、预测、隐变量参数、滤波后的原始数据、残差（用于初值约束）
        return recon, e_pred, (mu, logvar), x_smooth, x_delta


class TimeVAE1D_HybridMambaTransformer_Residual(nn.Module):
    """
    基于 Hybrid Mamba-Transformer 的时间序列 VAE - 残差重建版本
    
    核心改进：残差重建 + 初值约束
    - 解码器预测残差 r̂ = decoder(z)
    - 最终输出 x̂ = x_smooth + r̂ (残差叠加)
    - 初值约束损失 L_ic = ||r̂(t=0)||₁ ≈ 0
    ...
    def __init__(self, seq_len=200, C_in=1, latent_dim=64,
        ...
        # ------ Decoder: 预测残差，使用自适应上采样回到seq_len ------
        # 输入：latent_dim + 1（初始温度条件）
        self.fc_dec = nn.Linear(latent_dim + 1, decoder_base * 64)
